...
`,yd=Object.assign({"/posts/jellyfin-server.md":hd,"/posts/local-database.md":md,"/posts/using-commandline.md":gd,"/posts/using-git.md":bd,"/posts/variables-example.md":vd}),xd=e=>{const t=e.match(/^---\s*\n([\s\S]*?)\n---\s*\n([\s\S]*)$/);if(!t)return{frontmatter:{},content:e};const[,n,s]=t,o={},r=n.split(`
`);let i=null,a="";const l=(u,c)=>{c=c.trim(),c.startsWith("[")&&c.endsWith("]")?o[u]=c.slice(1,-1).split(",").map(d=>d.trim()):o[u]=c};return r.forEach(u=>{if(/^\s+/.test(u)&&!/^\s*\w+:/.test(u)&&i)a+=" "+u.trim();else{i&&a&&l(i,a);const[d,...m]=u.split(":");if(!d||d.trim()==="")return;i=d.trim(),a=m.join(":").trim()}}),i&&a&&l(i,a),{frontmatter:o,content:s}},_d=()=>{const e=[];let t=1;return Object.entries(yd).forEach(([n,s])=>{const{frontmatter:o,content:r}=xd(s),i=n.split("/").pop().replace(".md","");e.push({id:t++,slug:i,title:o.title||i,date:o.date||new Date().toISOString().split("T")[0],tags:o.tags||[],description:o.description||"",unlisted:o.unlisted===!0||o.unlisted==="true",content:r.trim(),readingTime:Sd(r)})}),e};let $n=null;const xo=(e=!1)=>($n||($n=_d()),(e?[...$n]:$n.filter(n=>!n.unlisted)).sort((n,s)=>qs(s.date)-qs(n.date))),wd=e=>xo(!0).find(t=>t.slug===e),Cd=()=>{const e=new Set;return xo().forEach(t=>{t.tags.forEach(n=>e.add(n))}),Array.from(e).sort()},qs=e=>{const[t,n,s]=e.split("-");return new Date(s,n-1,t)},Sd=e=>{const n=e.trim().split(/\s+/).length;return Math.ceil(n/225)},Ed={class:"border-l-2 border-catppuccin-surface pl-4"},Ad={class:"flex flex-wrap gap-2"},kd=["onClick"],Pd={__name:"TagFilter",props:{tags:{type:Array,default:()=>[]},selectedTag:{type:String,default:null}},emits:["toggle-tag"],setup(e,{emit:t}){const n=t,s=o=>{n("toggle-tag",o)};return(o,r)=>(O(),M("div",Ed,[r[0]||(r[0]=C("div",{class:"text-catppuccin-subtle text-sm mb-2"},"~$ ls tags/",-1)),C("div",Ad,[(O(!0),M(pe,null,Ge(e.tags,i=>(O(),M("button",{key:i,onClick:a=>s(i),class:Vt(["px-3 py-1 rounded text-xs transition-colors border",e.selectedTag===i?"bg-catppuccin-mauve/20 text-catppuccin-mauve border-catppuccin-mauve":"bg-catppuccin-base/40 text-catppuccin-subtle border-catppuccin-surface hover:text-catppuccin-text hover:border-catppuccin-overlay"])}," #"+X(i),11,kd))),128))])]))}},Td={class:"border-l-2 border-catppuccin-surface pl-4"},Rd={class:"text-catppuccin-subtle text-sm mb-3"},Id={key:0,class:"text-catppuccin-mauve"},Od={key:0,class:"text-sm text-catppuccin-subtle"},Dd={key:1,class:"space-y-3"},Ld=["onClick"],Md={class:"px-4 py-3"},$d={class:"flex items-start justify-between gap-4 mb-2"},Nd={class:"text-base font-semibold text-catppuccin-text group-hover:text-catppuccin-mauve transition-colors"},Fd={class:"flex items-center gap-2 flex-shrink-0"},Bd={class:"text-xs text-catppuccin-subtle"},Hd=["title"],Wd={class:"text-sm text-catppuccin-gray mb-3 leading-relaxed"},Vd={class:"flex items-center gap-2"},Ud={class:"flex flex-wrap gap-1.5"},jd=["onClick"],Gd={__name:"PostList",props:{posts:{type:Array,default:()=>[]},selectedTag:{type:String,default:null}},emits:["open-post","select-tag"],setup(e,{emit:t}){const n=t,s=o=>{n("open-post",o)};return(o,r)=>(O(),M("div",Td,[C("div",Rd,[r[0]||(r[0]=qe(" ~$ ls -la posts/ ",-1)),e.selectedTag?(O(),M("span",Id,'| grep "'+X(e.selectedTag)+'"',1)):Te("",!0)]),e.posts.length?(O(),M("div",Dd,[(O(!0),M(pe,null,Ge(e.posts,i=>(O(),M("div",{key:i.id,onClick:a=>s(i.slug),class:"block group rounded-md border border-catppuccin-surface/60 bg-catppuccin-base/20 hover:bg-catppuccin-base/30 hover:border-catppuccin-mauve/40 transition-all cursor-pointer"},[C("div",Md,[C("div",$d,[C("h2",Nd,X(i.title),1),C("div",Fd,[C("span",Bd,X(i.readingTime)+" min read ",1),r[1]||(r[1]=C("span",{class:"text-catppuccin-surface"},"•",-1)),C("span",{class:"text-xs text-catppuccin-subtle",title:Fe(qs)(i.date).toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"})},X(i.date),9,Hd)])]),C("p",Wd,X(i.description),1),C("div",Vd,[C("div",Ud,[(O(!0),M(pe,null,Ge(i.tags,a=>(O(),M("span",{key:a,onClick:Oc(l=>n("select-tag",a),["stop"]),class:"px-2 py-0.5 rounded text-xs bg-catppuccin-surface/60 text-catppuccin-subtle hover:bg-catppuccin-mauve/20 hover:text-catppuccin-mauve cursor-pointer transition-colors"}," #"+X(a),9,jd))),128))]),r[2]||(r[2]=C("span",{class:"ml-auto text-catppuccin-subtle group-hover:text-catppuccin-mauve transition-colors text-sm"}," read → ",-1))])])],8,Ld))),128))])):(O(),M("div",Od," no posts found "))]))}},qd={class:"mb-8"},Kd={class:"text-catppuccin-subtle text-sm mb-2"},zd={class:"text-3xl md:text-4xl font-bold text-catppuccin-text mb-3"},Yd={class:"flex items-center gap-4 text-sm text-catppuccin-subtle mb-4"},Jd={class:"flex gap-2"},Qd={key:0,class:"mb-6 border border-catppuccin-surface rounded-md p-4 bg-catppuccin-surface/10"},Zd={class:"space-y-3"},Xd=["for"],ep=["id","onUpdate:modelValue","placeholder"],tp={class:"border-l-2 border-catppuccin-surface pl-4 mb-8"},np=["innerHTML"],sp={__name:"PostComponent",props:{post:{type:Object,required:!0}},emits:["go-back"],setup(e,{emit:t}){const n=e,s=t,o=()=>{s("go-back")},r=ue(()=>n.post.readingTime||1),i=Ce({}),a=m=>{const p=new RegExp("(?<!\\\\)\\$\\[([^\\]]+)\\]","g"),b=new Set;let S;for(;(S=p.exec(m))!==null;)b.add(S[1]);return Array.from(b)},l=ue(()=>a(n.post.content)),u=m=>{const p=[];let b=m.replace(/\\\$\[([^\]]+)\]/g,(S,F)=>{const $=`__ESCAPED_VAR_${p.length}__`;return p.push(`$[${F}]`),$});return b=b.replace(/\$\[([^\]]+)\]/g,(S,F)=>i.value[F]||F),p.forEach((S,F)=>{b=b.replace(`__ESCAPED_VAR_${F}__`,S)}),b},c=ue(()=>u(n.post.content)),d=m=>{let p=m;const b=[];p=p.replace(/```(\w*)\s*\n?([\s\S]*?)```/g,(E,k,T)=>{const q=`__CODEBLOCK_${b.length}__`,z=T.trim().replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),Y=k?`language-${k.toLowerCase()}`:"",J=`code-block-${b.length}`;return b.push(`<div class="relative group">
            ...
            </div>`),q});const S=[];p=p.replace(/((?:\|[^\n]+\|\r?\n?)+)/g,E=>{const k=E.trim().split(/\r?\n/);if(k.length<2||!/^\|[\s\-:|]+\|$/.test(k[1]))return E;const q=`__TABLE_${S.length}__`,z=k[0],Y=k.slice(2);let J='<table class="w-full my-4 text-sm border-collapse">';const V=z.split("|").filter(K=>K.trim());return J+="<thead><tr>",V.forEach(K=>{J+=`<th class="border border-catppuccin-surface px-3 py-2 text-left text-catppuccin-mauve bg-catppuccin-surface/30">${K.trim()}</th>`}),J+="</tr></thead>",J+="<tbody>",Y.forEach(K=>{if(K.trim()&&!/^\|[\s\-:|]+\|$/.test(K)){const ae=K.split("|").filter(L=>L.trim());J+="<tr>",ae.forEach(L=>{J+=`<td class="border border-catppuccin-surface px-3 py-2 text-catppuccin-text">${L.trim()}</td>`}),J+="</tr>"}}),J+="</tbody></table>",S.push(J),q}),p=p.replace(/^(?:---|\*\*\*|___)\s*$/gim,'<hr class="border-catppuccin-surface my-6">');const F=E=>E.toLowerCase().replace(/<[^>]*>/g,"").replace(/[^\w\s-]/g,"").replace(/\s+/g,"-").replace(/-+/g,"-").trim();p=p.replace(/^### (.*$)/gim,(E,k)=>{const T=F(k);return`<h3 id="${T}" class="group text-lg font-semibold text-catppuccin-mauve mt-6 mb-3">${k}<a href="#${T}" class="ml-2 opacity-0 group-hover:opacity-100 transition-opacity text-catppuccin-subtle hover:text-catppuccin-mauve" aria-label="Link to this section">#</a></h3>`}),p=p.replace(/^## (.*$)/gim,(E,k)=>{const T=F(k);return`<h2 id="${T}" class="group text-xl font-semibold text-catppuccin-mauve mt-8 mb-4">${k}<a href="#${T}" class="ml-2 opacity-0 group-hover:opacity-100 transition-opacity text-catppuccin-subtle hover:text-catppuccin-mauve" aria-label="Link to this section">#</a></h2>`}),p=p.replace(/^# (.*$)/gim,(E,k)=>{const T=F(k);return`<h1 id="${T}" class="group text-2xl font-bold text-catppuccin-mauve mt-8 mb-4">${k}<a href="#${T}" class="ml-2 opacity-0 group-hover:opacity-100 transition-opacity text-catppuccin-subtle hover:text-catppuccin-mauve" aria-label="Link to this section">#</a></h1>`}),p=p.replace(/^> (.*$)/gim,'<blockquote class="border-l-4 border-catppuccin-mauve pl-4 py-2 my-4 text-catppuccin-text italic bg-catppuccin-surface/20">$1</blockquote>'),p=p.replace(/!\[([^\]]*)\]\(([^)]+)\)/g,'<img src="$2" alt="$1" class="max-w-full h-auto rounded my-4">'),p=p.replace(/\*\*\*(.*?)\*\*\*/g,'<strong class="text-catppuccin-mauve font-semibold"><em>$1</em></strong>'),p=p.replace(/\*\*(.*?)\*\*/g,'<strong class="text-catppuccin-mauve font-semibold">$1</strong>'),p=p.replace(/\*(.*?)\*/g,'<em class="text-catppuccin-text italic">$1</em>'),p=p.replace(/~~(.*?)~~/g,'<del class="text-catppuccin-subtle line-through">$1</del>'),p=p.replace(/`([^`]+)`/g,'<code class="bg-catppuccin-surface/50 px-2 py-0.5 rounded text-catppuccin-pink text-sm">$1</code>'),p=p.replace(/\[([^\]]+)\]\(([^)]+)\)/g,'<a href="$2" target="_blank" class="text-catppuccin-mauve hover:text-catppuccin-mauve underline transition-colors">$1</a>'),p=p.replace(/^\d+\. (.*$)/gim,'<li class="ml-6 list-decimal text-catppuccin-text mb-1">$1</li>'),p=p.replace(/^[\-\*\+] (.*$)/gim,'<li class="ml-6 list-disc text-catppuccin-text mb-1">$1</li>'),p=p.replace(/^[\-\*\+] \[ \] (.*$)/gim,'<li class="ml-6 list-none text-catppuccin-text mb-1"><input type="checkbox" disabled class="mr-2">$1</li>'),p=p.replace(/^[\-\*\+] \[x\] (.*$)/gim,'<li class="ml-6 list-none text-catppuccin-text mb-1"><input type="checkbox" checked disabled class="mr-2">$1</li>');const $=/^<(h[1-6]|ul|ol|li|blockquote|pre|div|hr|table|thead|tbody|tr|th|td)/i;return p=p.split(`

`).map(E=>{const k=E.trim();if(k.length===0||k.startsWith("__CODEBLOCK_")||k.startsWith("__TABLE_"))return E;const T=E.split(`
`),q=[];let z=[];const Y=()=>{if(z.length>0){const J=z.join("<br>");q.push(`<p class="text-catppuccin-text leading-relaxed mb-4">${J}</p>`),z=[]}};return T.forEach(J=>{const V=J.trim();V.length===0||$.test(V)||V.startsWith("__CODEBLOCK_")||V.startsWith("__TABLE_")?(Y(),q.push(J)):z.push(J.trim())}),Y(),q.join(`
`)}).join(`

`),b.forEach((E,k)=>{p=p.replace(`__CODEBLOCK_${k}__`,E)}),S.forEach((E,k)=>{p=p.replace(`__TABLE_${k}__`,E)}),p};return Ut(()=>{setTimeout(()=>{window.Prism&&(Prism.highlightAll(),document.querySelectorAll('pre[class*="language-"]').forEach(m=>{m.className=m.className.replace(/language-\S+/g,"").trim()}))},100)}),(m,p)=>(O(),M("div",null,[C("div",qd,[C("div",Kd," ~$ cat "+X(e.post.slug)+".md ",1),C("button",{onClick:o,class:"text-sm text-catppuccin-subtle hover:text-catppuccin-text transition-colors mb-6 inline-flex items-center gap-1"}," [← back to posts] "),C("h1",zd,X(e.post.title),1),C("div",Yd,[C("span",null,X(e.post.date),1),p[0]||(p[0]=C("span",{class:"text-catppuccin-surface"},"•",-1)),C("span",null,"~"+X(r.value)+" min read",1),p[1]||(p[1]=C("span",{class:"text-catppuccin-surface"},"•",-1)),C("div",Jd,[(O(!0),M(pe,null,Ge(e.post.tags,b=>(O(),M("span",{key:b,class:"text-catppuccin-gray"}," #"+X(b),1))),128))])])]),l.value.length>0?(O(),M("div",Qd,[p[2]||(p[2]=C("div",{class:"text-sm text-catppuccin-subtle mb-3"}," ~$ configure variables ",-1)),C("div",Zd,[(O(!0),M(pe,null,Ge(l.value,b=>(O(),M("div",{key:b,class:"flex items-center gap-3"},[C("label",{for:`var-${b}`,class:"text-sm text-catppuccin-text min-w-[120px]"},X(b)+": ",9,Xd),Qa(C("input",{id:`var-${b}`,"onUpdate:modelValue":S=>i.value[b]=S,type:"text",placeholder:b,class:"flex-1 px-3 py-1.5 text-sm bg-catppuccin-base border border-catppuccin-surface/60 rounded text-catppuccin-text placeholder-catppuccin-subtle focus:outline-none focus:border-catppuccin-mauve transition-colors"},null,8,ep),[[Tc,i.value[b]]])]))),128))])])):Te("",!0),C("article",tp,[C("div",{class:"prose prose-invert max-w-none text-catppuccin-text",innerHTML:d(c.value)},null,8,np)]),C("button",{onClick:o,class:"text-sm text-catppuccin-subtle hover:text-catppuccin-mauve transition-colors inline-flex items-center gap-1"}," [← back to all posts] ")]))}},op=In(sp,[["__scopeId","data-v-f96a9311"]]),rp={class:"w-full min-h-screen h-screen overflow-x-hidden overflow-y-auto font-mono"},ip={class:"max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-16"},ap={key:"list"},lp={class:"mb-12"},cp={class:"flex items-center gap-4 text-sm mb-6"},up={key:"post"},fp={__name:"Posts",setup(e){const t=Ce("list"),n=Ce(null),s=Ce(null),o=Ce([]),r=Ce([]),i=ia(),a=vo(),l=ue(()=>s.value?o.value.filter(p=>p.tags.includes(s.value)):o.value),u=()=>{o.value=xo(),r.value=Cd()},c=p=>{if(n.value=wd(p),n.value)t.value="post",window.scrollTo({top:0,behavior:"smooth"}),i.query.post!==p&&a.replace({name:"Posts",query:{...i.query,post:p}});else if(i.query.post){const b={...i.query};delete b.post,a.replace({name:"Posts",query:b})}},d=({skipQueryUpdate:p=!1}={})=>{if(t.value="list",n.value=null,window.scrollTo({top:0,behavior:"smooth"}),!p&&"post"in i.query){const b={...i.query};delete b.post,a.replace({name:"Posts",query:b})}},m=p=>{s.value=s.value===p?null:p};return Ut(()=>{u(),document.documentElement.style.overflowY="auto",document.body.style.overflowY="auto",new ClipboardJS("[data-clipboard-target]").on("success",function(S){const F=S.trigger,$=F.textContent;F.textContent="copied!",F.classList.add("text-catppuccin-green"),setTimeout(()=>{F.textContent=$,F.classList.remove("text-catppuccin-green")},2e3),S.clearSelection()}),setTimeout(()=>{window.Prism&&Prism.highlightAll()},100);const b=i.query.post;b&&c(b)}),as(()=>{document.documentElement.style.overflowY="",document.body.style.overflowY=""}),hn(()=>i.query.post,(p,b)=>{p&&p!==b?c(p):!p&&t.value==="post"&&d({skipQueryUpdate:!0})}),(p,b)=>{const S=ao("router-link");return O(),M("div",rp,[C("div",ip,[te(mo,{name:"fade",mode:"out-in"},{default:Pt(()=>[t.value==="list"?(O(),M("div",ap,[C("div",lp,[b[1]||(b[1]=C("div",{class:"text-catppuccin-subtle text-sm mb-2"},"~$ cd ~/posts",-1)),b[2]||(b[2]=C("h1",{class:"text-3xl md:text-4xl font-bold text-catppuccin-text mb-4"},[C("span",{class:"text-catppuccin-mauve"},"Posts")],-1)),b[3]||(b[3]=C("p",{class:"text-sm text-catppuccin-gray leading-relaxed mb-6"}," My thoughts, tutorials, and experiences on various topics including web development, programming, and technology. ",-1)),C("div",cp,[te(S,{to:"/",class:"text-catppuccin-subtle hover:text-catppuccin-text transition-colors"},{default:Pt(()=>[...b[0]||(b[0]=[qe(" [← home] ",-1)])]),_:1})]),te(Pd,{tags:r.value,"selected-tag":s.value,onToggleTag:m},null,8,["tags","selected-tag"])]),te(Gd,{posts:l.value,"selected-tag":s.value,onOpenPost:c,onSelectTag:m},null,8,["posts","selected-tag"]),te(Qn)])):t.value==="post"&&n.value?(O(),M("div",up,[te(op,{post:n.value,onGoBack:d},null,8,["post"]),te(Qn)])):Te("",!0)]),_:1})])])}}},dp=In(fp,[["__scopeId","data-v-7f74cefd"]]),pp={class:"w-full min-h-screen h-screen overflow-x-hidden overflow-y-auto font-mono"},hp={class:"max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 md:pt-16"},mp={class:"mb-8"},gp={class:"text-catppuccin-subtle text-sm mb-4"},bp={class:"border-l-2 border-catppuccin-surface pl-4"},vp={class:"text-catppuccin-red text-sm"},yp={class:"text-catppuccin-mauve"},xp={__name:"NotFound",setup(e){const t=ia(),n=vo(),s=ue(()=>(t.fullPath||t.path||"/").replace(/^\//,"")||"."),o=()=>n.push("/");return(r,i)=>(O(),M("div",pp,[C("div",hp,[C("div",mp,[C("div",gp," ~$ cd ~/"+X(s.value),1),C("div",{class:"flex items-center gap-4 text-sm mb-6"},[C("button",{onClick:o,class:"text-catppuccin-subtle hover:text-catppuccin-text transition-colors"}," [← home] ")])]),C("div",bp,[C("div",vp,[i[0]||(i[0]=qe(" cd: no such file or directory: /",-1)),C("span",yp,X(s.value),1)])])]),te(Qn)]))}},_p=[{path:"/",name:"Home",component:pd,meta:{title:"Home | heckr.dev"}},{path:"/posts",name:"Posts",component:dp,meta:{title:"Posts | heckr.dev"}},{path:"/:pathMatch(.*)*",name:"NotFound",component:xp,meta:{title:"404 Not Found | heckr.dev"}}],la=Uu({history:_u(),routes:_p,scrollBehavior(e,t,n){return n||{top:0}}});la.beforeEach((e,t,n)=>{document.title=e.meta.title||"heckr.dev",n()});let cn=0;const Ar=["ArrowUp","ArrowUp","ArrowDown","ArrowDown","ArrowLeft","ArrowRight","ArrowLeft","ArrowRight","KeyB","KeyA"],wp=()=>{console.log("%cWelcome to heckr.dev","font-size: 20px; font-weight: bold; color: #cba6f7;"),console.log("%cWelcome to the dev console, here are some commands to try:","font-size: 14px; color: #a6adc8;"),console.log(`%c- help() - show available commands
...
Feel free to reach out!`,"font-size: 12px; color: #a6adc8;")},window.secret=()=>{console.log("%cYou found the secret command","font-size: 18px; font-weight: bold; color: #f9e2af;"),console.log("%cHere's a hint: ↑ ↑ ↓ ↓ ← → ← → B A","font-size: 12px; color: #fab387;")},document.addEventListener("keydown",e=>{e.code===Ar[cn]?(cn++,cn===Ar.length&&(Cp(),cn=0)):cn=0})},Cp=()=>{if(console.log("%cKONAMI CODE ACTIVATED!","font-size: 24px; font-weight: bold; color: #f9e2af; text-shadow: 2px 2px 4px #000;"),document.body.style.animation="rainbow-border 2s linear infinite",!document.getElementById("konami-style")){const e=document.createElement("style");e.id="konami-style",e.textContent=`
    ...
    `,document.head.appendChild(e)}setTimeout(()=>{document.body.style.animation=""},5e3)};Mc(ju).use(la).mount("#app");wp();
